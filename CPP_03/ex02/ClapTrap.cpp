...
ClapTrap::ClapTrap() : _hitPoints(10), _energy(10), _attackDmg(0)
{
...
void ClapTrap::doAttack(const std::string &type, const std::string &target)
{
	if (_energy > 0 && _hitPoints > 0)
	{
		_energy--;
		std::cout << type + " " + _name + " attack " + target + ", causing ";
		std::cout << _attackDmg << " points of damage!" << std::endl;
	}
	else if (_energy == 0)
		std::cout << type + " " + _name + " can't attack, no energy! " << std::endl;
	else
		std::cout << type + " " + _name + " can't attack, no hit points! " << std::endl;
}

void ClapTrap::attack(const std::string &target)
{
	doAttack("ClapTrap", target);
}
